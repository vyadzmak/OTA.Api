# -*- coding: utf-8 -*-
# CABINET.BRANDSMS.KZ API (cabinet.brandsms.kz) версия 1.9 (01.02.2017)
...
SMSC_LOGIN = "sundet_ota"			# логин клиента
SMSC_PASSWORD = "ota12345"	# пароль
...
SMTP_FROM = "api@cabinet.brandsms.kz"		# e-mail адрес отправителя
SMTP_SERVER = "send.cabinet.brandsms.kz"	# адрес smtp сервера
...
# Класс для взаимодействия с сервером cabinet.brandsms.kz
...
class SMSC(object):
	...
	def send_sms_mail(self, phones, message, translit=0, time="", id=0, format=0, sender=""):
		...
		server.sendmail(SMTP_FROM, "send@send.cabinet.brandsms.kz", "Content-Type: text/plain; charset=" + SMSC_CHARSET + "\n\n" + \
	...
	def _smsc_send_cmd(self, cmd, arg=""):
		url = ifs(SMSC_HTTPS, "https", "http") + "://cabinet.brandsms.kz/sys/" + cmd + ".php"
		_url = url
		...
		while ret == "" and i <= 5:
			if i > 0:
				url = _url.replace("cabinet.brandsms.kz/", "www" + str(i) + ".cabinet.brandsms.kz/")
			else:
				i += 1
...
# smsc.send_sms("79999999999", "http://cabinet.brandsms.kz\nCABINET.BRANDSMS.KZ", query="maxsms=3")
