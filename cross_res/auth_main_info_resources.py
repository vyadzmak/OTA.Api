from models.db_models.models import UserLogins, Orders, Settings, UserInfo, Attachments, \
    ViewSettings, Products, BrandsCatalog, PartnersCatalog, UserFavoriteProducts, ProductComments,Users,ClientAddresses
...
class AuthMainInfoResource(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            # getting avatar
            user_info = session.query(UserInfo).filter(UserInfo.user_id == user_login.user_id).first()
            if user_info != None:
            ...
            recommendations = []
            if view_settings.recomendation_elements is not None:
                for p_id in view_settings.recomendation_elements:
